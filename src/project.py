...
def fix_overlaps(job):
    ...
    return "cd {} ; aprun -n 16 lmp_titan -in {}/in.minimize -log {}/minimize.log".format(
...
def lmp_to_gmx(job):
    ...
    msg = ("cd {} ; aprun -n 1 gmx_mpi trjconv -s init.gro -f minimize.xtc \
    -o minimize.gro  -b 1.0 -e 1.0 < {}".format(job.workspace(), str(p.absolute())))
    return msg
...
def mdrun_shear_5nN(job):
    ...
    return "cd {}; aprun -n 16 gmx_mpi mdrun -s shear_5nN.tpr -deffnm shear_5nN -cpi shear_5nN.cpt -cpo shear_5nN.cpt -noappend -ntomp 1 -gpu_id 0000000000000000".format(job.workspace())
...
def mdrun_shear_15nN(job):
    ...
    return "cd {}; aprun -n 16 gmx_mpi mdrun -s shear_15nN.tpr -deffnm shear_15nN -cpi shear_15nN.cpt -cpo shear_15nN.cpt -noappend -ntomp 1 -gpu_id 0000000000000000".format(job.workspace())
    #return "cd {}; aprun -n 16 {} -append".format(job.workspace(), msg)
...
def mdrun_shear_25nN(job):
    ...
    return "cd {}; aprun -n 16 gmx_mpi mdrun -s shear_25nN.tpr -deffnm shear_25nN -cpi shear_25nN.cpt -cpo shear_25nN.cpt -noappend -ntomp 1 -gpu_id 0000000000000000".format(job.workspace())
    #return "cd {}; aprun -n 16 {} -append".format(job.workspace(), msg)
